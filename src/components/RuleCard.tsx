  ...
  if (rule.scopeKind === 'all') {
    return (
      <View className="rounded bg-surface-secondary px-1.5 py-0.5 dark:bg-surface-dark-secondary">
        <Text className="text-[11px] text-muted dark:text-muted-dark">All apps</Text>
      </View>
    );
  }
  ...
  return (
    ...
    >
      ...
      <View className="flex-1 gap-1.5">
        {rule.label ? (
          <Text className="text-sm font-medium text-surface-dark dark:text-white" numberOfLines={1}>
            {rule.label}
          </Text>
        ) : null}
        ...
        <Text className="font-mono text-xs text-muted dark:text-muted-dark" numberOfLines={1}>
          {rule.pattern}
        </Text>
      </View>
